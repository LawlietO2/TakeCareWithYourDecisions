	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag (VertOut i) : SV_Target
			{
				...
				if( _ScanDistance < 1.75 &&  _ScanDistance > dist && dist > _ScanDistance - _ScanWidth && linearDepth < 1 ){
				
					float diff = 1 - (_ScanDistance - dist) / (_ScanWidth);
					half4 edge = lerp(_MidColor, _LeadColor, 0.1);
					scannerCol = lerp(_TrailColor, edge, diff) + 0.1 * _HBarColor;
					scannerCol *= diff;
